...
mcp_server/server.py
 FastMCP server exposing 7 tools for the e-commerce support agent.
...
def get_order(order_id: str, customer_id: Optional[str] = None) -> dict:
    ...
    Verify the order belongs to the authenticated customer before returning it.
    ...
    if not customer_id:
        conn.close()
        return {"found": False, "error": "Authenticated customer ID is required."}
...
@mcp.tool()
def list_orders(customer_id: Optional[str] = None) -> dict:
    """List every order owned by the authenticated customer."""
    if not customer_id:
        return {"found": False, "orders": [], "error": "Authenticated customer ID is required."}
    ...
    conn.close()
    return {"found": True, "orders": [dict(row) for row in rows]}


# Tool 4: request_return
# ---------------------------------------------------------------------------

@mcp.tool()
def request_return(order_id: str, customer_id: str) -> dict:
    """Create a return request for an owned order."""
    if not customer_id:
        return {"success": False, "error": "Authenticated customer ID is required."}
    ...
    if row is None:
        conn.close()
        return {"success": False, "error": f"Order {order_id} was not found for this customer."}
    if row["status"] == "return_requested":
        conn.close()
        return {"success": False, "error": f"A return request already exists for order {order_id}."}
    ...
    conn.execute(
        "UPDATE orders SET status = 'return_requested', updated_at = ? WHERE order_id = ? AND customer_id = ?",
    ...
    return {
        ...
        "message": (
            f"Your return request for order {order_id} has been created. "
