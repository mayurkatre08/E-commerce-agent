...
class _SmartFakeChatModel(BaseChatModel):
    ...
    def _generate(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        if '"intent"' in combined or "respond only with valid json" in combined:
            if any(w in user_text for w in ["return", "refund"]):
                order_id = "ORD9001" if "ord9" in user_text else None
                text = f'{{"intent": "return", "customer_id": "C001", "order_id": {json.dumps(order_id)}}}'
            elif any(w in user_text for w in ["cancel", "cancell"]):
                ...
                order_id = "ORD9002" if "ord9" in user_text else None
                text = f'{{"intent": "order_lookup", "customer_id": "C002", "order_id": {json.dumps(order_id)}}}'
