...
with st.sidebar:
    ...
    if not st.session_state.access_token:
        login_customer_id = st.text_input("Customer ID", placeholder="e.g. C001")
        login_email = st.text_input("Email", placeholder="e.g. alice@example.com")
        if st.button("Log in", type="primary"):
            try:
                response = requests.post(
                    f"{API_URL}/auth/token",
                    json={"customer_id": login_customer_id, "email": login_email},
                    timeout=5,
                )
...
user_input = st.chat_input(
    ...
    disabled=not st.session_state.access_token or st.session_state.escalated,
)
